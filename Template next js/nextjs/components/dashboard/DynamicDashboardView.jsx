...
export default function DynamicDashboardView({ role = 'client' }) {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-0 pb-10">
            ...
            {isMismatch && (
                <div className="mb-8 p-6 bg-amber-50 border-2 border-amber-200 rounded-[2rem] flex flex-col md:flex-row items-center gap-6 text-amber-900 shadow-xl shadow-amber-900/5">
                    <div className="w-12 h-12 bg-amber-200 rounded-2xl flex items-center justify-center shrink-0">
                        <AlertCircle className="w-6 h-6" />
                    </div>
                    <div className="flex-1">
                        <p className="font-black uppercase tracking-widest text-[11px] mb-1">Configuration de rôle incohérente</p>
                        <p className="text-xs font-medium italic opacity-80">
                            HOLA détecte que votre rôle en base de données est <span className="font-bold underline">"{profileRole}"</span>,
                            mais vous visualisez l'espace <span className="font-bold underline">"{role}"</span>.
                        </p>
                    </div>
                    <button
                        onClick={async () => {
                            const { data: { user } } = await supabase.auth.getUser();
                            const { error } = await supabase.from('profiles').update({ role: role }).eq('id', user.id);
                            if (!error) window.location.reload();
                            else alert(error.message);
                        }}
                        className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg active:scale-95 flex items-center gap-2"
                    >
                        Fixer mon rôle à "{role}"
                    </button>
                </div>
            )}
            ...
            <div className="space-y-8">
                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-10 shadow-sm relative overflow-hidden">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase leading-tight">Activités récentes</h2>
                            <p className="text-slate-400 text-[10px] font-black mt-1.5 uppercase tracking-widest italic opacity-70">Flux de vos dernières opérations</p>
                        </div>
                        <button
                            onClick={() => setIsModalOpen(true)}
                            className="w-full md:w-auto px-6 py-4 md:py-3 bg-slate-50 hover:bg-slate-900 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center md:justify-start gap-2 transition-all shadow-sm active:scale-95 border border-slate-100/50"
                        >
